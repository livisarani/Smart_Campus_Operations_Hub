...
const BookingForm = ({ onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    ...
    const loadResources = async () => {
      try {
        ...
        if (!isActive) return;
        setResources([]);
        setResourceLoadError('Unable to load resources from the server.');
      } finally {
        if (isActive) {
  ...
  const handleResourceChange = (e) => {
    ...
    const selectedResource = resources.find((resource) => Number(resource.id) === selectedId);
          ...
          >
            ...
            {resources.map((resource) => (
              <option key={resource.id} value={resource.id}>
                {resource.name} ({resource.type}){resource.available === false ? ' - Unavailable' : ''}
              </option>
            ))}
          </select>
          {resourceLoadError && <small className="resource-id-hint">{resourceLoadError}</small>}
          {formData.resourceId && (
            <small className="resource-id-hint">Resource ID: {formData.resourceId}</small>
          )}
        </div>
